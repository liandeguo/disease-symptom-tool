import { colorPanelsMeta, defaultPatternSizing, dotOrbitMeta, emptyPixel, gemSmokeMeta, godRaysMeta, grainGradientMeta, heatmapMeta, toProcessedGemSmoke, toProcessedHeatmap, toProcessedLiquidMetal, meshGradientMeta, metaballsMeta, pulsingBorderMeta, simplexNoiseMeta, smokeRingMeta, staticMeshGradientMeta, staticRadialGradientMeta, swirlMeta, voronoiMeta, warpMeta, DitheringShapes, DitheringTypes, DotGridShapes, GemSmokeShapes, GlassDistortionShapes, GlassGridShapes, GrainGradientShapes, HalftoneCmykTypes, HalftoneDotsGrids, HalftoneDotsTypes, LiquidMetalShapes, PulsingBorderAspectRatios, WarpPatterns, getShaderColorFromString, ShaderFitOptions } from '@paper-design/shaders';
...
export function toDitheringUniforms(params) {
    const sizing = {
...
export function toPaperTextureUniforms(params, assets) {
    const sizing = {
        ...defaultPatternSizing,
...
export function toFlutedGlassUniforms(params, assets) {
    const sizing = {
        ...defaultPatternSizing,
        fit: params.fit ?? 'cover',
        scale: params.scale ?? defaultPatternSizing.scale,
...
export function toWaterUniforms(params, assets) {
    const sizing = {
        ...defaultPatternSizing,
...
export function toImageDitheringUniforms(params, assets) {
    const sizing = {
        ...defaultPatternSizing,
        fit: params.fit ?? 'cover',
        scale: params.scale ?? defaultPatternSizing.scale,
...
export function toHalftoneDotsUniforms(params, assets) {
    const sizing = {
        ...defaultPatternSizing,
        fit: params.fit ?? 'cover',
        scale: params.scale ?? defaultPatternSizing.scale,
...
export function toHalftoneCMYKUniforms(params, assets) {
    const sizing = {
        ...defaultPatternSizing,
        fit: params.fit ?? 'cover',
        scale: params.scale ?? defaultPatternSizing.scale,
...
export function toMeshGradientUniforms(params) {
    const sizing = {
        ...defaultPatternSizing,
        fit: params.fit ?? 'contain',
        scale: params.scale ?? defaultPatternSizing.scale,
...
export function toStaticMeshGradientUniforms(params) {
    const sizing = {
        ...defaultPatternSizing,
        fit: params.fit ?? 'contain',
        scale: params.scale ?? defaultPatternSizing.scale,
    ...
    const source = params.colors && params.colors.length > 0 ? params.colors : staticMeshGradientDefaultColors;
...
export function toStaticRadialGradientUniforms(params) {
    const sizing = {
        ...defaultPatternSizing,
        fit: params.fit ?? 'contain',
        scale: params.scale ?? defaultPatternSizing.scale,
    ...
    const source = params.colors && params.colors.length > 0 ? params.colors : staticRadialGradientDefaultColors;
...
export function toGrainGradientUniforms(params, assets) {
    const sizing = {
        ...defaultPatternSizing,
        fit: params.fit ?? 'contain',
        scale: params.scale ?? defaultPatternSizing.scale,
    ...
    const source = params.colors && params.colors.length > 0 ? params.colors : grainGradientDefaultColors;
...
export function toDotOrbitUniforms(params, assets) {
    const sizing = {
        ...defaultPatternSizing,
        fit: params.fit ?? defaultPatternSizing.fit,
        scale: params.scale ?? defaultPatternSizing.scale,
    ...
    const source = params.colors && params.colors.length > 0 ? params.colors : dotOrbitDefaultColors;
...
export function toDotGridUniforms(params) {
    const sizing = {
        ...defaultPatternSizing,
        fit: params.fit ?? defaultPatternSizing.fit,
        scale: params.scale ?? defaultPatternSizing.scale,
...
export function toWarpUniforms(params, assets) {
    const sizing = {
        ...defaultPatternSizing,
        fit: params.fit ?? defaultPatternSizing.fit,
        scale: params.scale ?? defaultPatternSizing.scale,
...
export function toSpiralUniforms(params) {
    const sizing = {
        ...defaultPatternSizing,
        fit: params.fit ?? 'contain',
        scale: params.scale ?? defaultPatternSizing.scale,
...
export function toSwirlUniforms(params) {
    const sizing = {
        ...defaultPatternSizing,
        fit: params.fit ?? 'contain',
        scale: params.scale ?? defaultPatternSizing.scale,
...
export function toWavesUniforms(params) {
    const sizing = {
        ...defaultPatternSizing,
        fit: params.fit ?? defaultPatternSizing.fit,
        scale: params.scale ?? 0.6,
...
export function toNeuroNoiseUniforms(params) {
    const sizing = {
        ...defaultPatternSizing,
        fit: params.fit ?? defaultPatternSizing.fit,
        scale: params.scale ?? defaultPatternSizing.scale,
...
export function toPerlinNoiseUniforms(params) {
    const sizing = {
        ...defaultPatternSizing,
        fit: params.fit ?? defaultPatternSizing.fit,
        scale: params.scale ?? defaultPatternSizing.scale,
...
export function toSimplexNoiseUniforms(params) {
    const sizing = {
        ...defaultPatternSizing,
        fit: params.fit ?? defaultPatternSizing.fit,
    ...
    const source = params.colors && params.colors.length > 0 ? params.colors : simplexNoiseDefaultColors;
...
export function toVoronoiUniforms(params, assets) {
    const sizing = {
        ...defaultPatternSizing,
        fit: params.fit ?? defaultPatternSizing.fit,
...
export function toPulsingBorderUniforms(params, assets) {
    const sizing = {
        ...defaultPatternSizing,
        fit: params.fit ?? 'contain',
        scale: params.scale ?? 0.6,
    ...
    const margin = params.margin ?? 0;
    const source = params.colors && params.colors.length > 0 ? params.colors : pulsingBorderDefaultColors;
...
export function toMetaballsUniforms(params, assets) {
    const sizing = {
        ...defaultPatternSizing,
        fit: params.fit ?? 'contain',
        scale: params.scale ?? defaultPatternSizing.scale,
    ...
    const source = params.colors && params.colors.length > 0 ? params.colors : metaballsDefaultColors;
...
export function toColorPanelsUniforms(params) {
    const sizing = {
        ...defaultPatternSizing,
        fit: params.fit ?? 'contain',
    ...
    const source = params.colors && params.colors.length > 0 ? params.colors : colorPanelsDefaultColors;
...
export function toSmokeRingUniforms(params, assets) {
    const sizing = {
        ...defaultPatternSizing,
        fit: params.fit ?? 'contain',
        scale: params.scale ?? 0.8,
    ...
    const source = params.colors && params.colors.length > 0 ? params.colors : smokeRingDefaultColors;
...
export function toGodRaysUniforms(params, assets) {
    const sizing = {
        ...defaultPatternSizing,
        fit: params.fit ?? 'contain',
        scale: params.scale ?? defaultPatternSizing.scale,
...
export function toHeatmapUniforms(params, assets) {
    const sizing = {
        ...defaultPatternSizing,
        fit: params.fit ?? 'contain',
...
export function toLiquidMetalUniforms(params, assets) {
    const sizing = {
        ...defaultPatternSizing,
        fit: params.fit ?? 'contain',
        scale: params.scale ?? 0.6,
...
export function toGemSmokeUniforms(params, assets) {
    const sizing = {
        ...defaultPatternSizing,
        fit: params.fit ?? 'contain',
        scale: params.scale ?? 0.6,
...
export async function processShaderUniforms(uniformsProp) {
...
// ─────────────────────────────────────────────────────────────────────────────
const heatmapImageCache = new Map();
const liquidMetalImageCache = new Map();
const gemSmokeImageCache = new Map();
function loadProcessedImage(source, cache, process) {
    ...
    const promise = (async () => {
        ...
        if (!blob)
            throw new Error(`Processing returned no blob for ${source}`);
...
export function preloadHeatmap(source) {
...
export function preloadLiquidMetal(source) {
...
export function preloadGemSmoke(source) {
...
let emptyPixelImagePromise = null;
export function getEmptyPixelImage() {
    if (emptyPixelImagePromise)
